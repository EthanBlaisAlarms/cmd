...
connectRetry = cmd.RegRead("HKLM\SOFTWARE\EBA-Cmd\timesToAutoRetryInternetConnection")
...
Do
	...
	Elseif startupType = "repair" Then
		...
		programLoc = cmd.RegRead("HKLM\SOFTWARE\EBA-Cmd\installDir")
		Err.Clear
		eba = msgbox("Are you sure you want to run automatic repair? Running automatic repair will reset your preferences. You may also need to re-add your EBA Key.",4+48+4096,title)
		If eba = vbYes Then
	...
	Elseif startupType = "refresh" Then
		...
		temp(0) = "no"
		temp(1) = "no"
		eba = LCase(inputbox("Enter reinstallation type:" & line & "'KEEP': Preserves all local data. Rewrites EBA-Cmd to the registry." & vblf & "'REFRESH': Preserves commands." & vblf & "'FULL': Does not preserve data.",title,"refresh"))
		If eba = "keep" or eba = "refresh" or eba = "full" Then
			...
			Elseif eba = "refresh" Then
				temp(1) = "yes"
			End If
			eba = msgbox("Preserved data:" & line & "Script: yes" & vblf & "EBA-Cmd Registry: no" & vblf & "Users: " & temp(0) & vblf & "Commands: " & temp(1) & vblf & "Settings: " & temp(0) & line & "Proceed? This cannot be undone", 4+48+4096, title)
			If eba = vbNo Then
				Call write(dataLoc & "\startupType.ebacmd","normal")
				Call endOp("c")
			End If
