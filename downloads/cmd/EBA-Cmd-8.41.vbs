...
If fExists(cmd.SpecialFolders("Startup") & "\uninstallEBA.vbs") Then
	Call giveError(localization(0),"UNINSTALLION_SCHEDULED")
	Call endOp("c")
End If
...
If InStr(temp(0),"microsoft") Then
	If InStr(temp(0),"windows") Then
		...
		Elseif InStr(temp(0),"vista") Then
			Call giveWarn(localization(1))
		Else
			Call giveError(localization(2),"Invalid_Windows_Version")
		End If
	Else
		Call giveError(localization(3),"Windows_RE")
		Call endOp("c")
	End If
Else
	Call giveError(localization(4),"INVALID_OS")
	Call endOp("c")
End If
...
If scriptRunning() Then
	Call giveError(localization(0),"EBA_ALCall readY_RUNNING")
	Call endOp("s")
End If
If checkCScript() Then
	Call giveError(localization(0),"USE_WSCRIPT_NOT_CSCRIPT")
	Call endOp("s")
End If
	...
	Do
		...
		If exeValue = "eba.admin" Then
			If isAdmin = False Then
				Call endOp("ra")
			End If
			Call giveNote("EBA Command Center is alCall ready running as administrator.")
			...
			If eba = vbYes Then
				...
				If eba = "user" or eba = "cmd" or eba = "settings" or eba = "plug" Then
					...
					Elseif eba = "plug" Then
						...
						backup1.CopyHere backup2.items, 4
						If Err.Number = 0 Then
							Call giveNote("Backed up all files in " & dataLoc & "\Plugins")
						Else
							Call giveError("Backup failed. See WScript Error for more info.","WS/" & Err.Number & "?Mode=BackupError")
						End If
						Call checkWScript
			...
			If eba = "cmd" Then
				...
				If eba = "new" Then
					...
					If fExists(dataLoc & "\Commands\" & eba & ".ebacmd") or fExists(programLoc & "\Commands\" & eba & ".ebacmd") Then
						Call giveError("That command alCall ready exists.","DUPLICATE_FILE_DETECTED")
				...
				If eba = "new" Then
					...
					If temp(0) < temp(1) Then
						...
						If fExists(dataLoc & "\Users\" & eba & ".ebacmd") Then
							Call giveError("That user alCall ready exists.","DUPLICATE_FILE_DETECTED")
				...
				If fExists(forVar & "\meta.xml") Then
					...
					For Each forVar1 In objXML.selectNodes("/Meta/Format")
						...
						If forVar1.text = "1" Then
							...
							Next
						Else
							Call giveError("The plugin at " & forVar & " contains an invalid META.XML file, and will not be displayed.","UNKNOWN_FORMAT_VERSION")
						End If
					Next
					
				Else
					Call giveWarn("The plugin at " & forVar & " is missing META.XML, and will not be displayed.")
				End If
			Next
	...
	If LCase(Right(importData, 10)) = ".ebaimport" Or LCase(Right(importData, 10)) = ".ebabackup" Or LCase(Right(importData, 10)) = ".ebaplugin" Then
		If LCase(Right(importData, 10)) = ".ebaimport" Then
			...
			If lines(1) = "Type: Startup Key" Then
				...
				Else
					Call giveError("Something is wrong with that file. EBA Command Center couldn't Call read it or couldn't understand its contents: " & importData,"INVALID_STARTUP_KEY")
				End If
			Elseif lines(1) = "Type: Command" Then
				...
				If eba = vbYes Then
					If fExists(dataLoc & "\Commands\" & lines(2) & ".ebacmd") Then
						Call giveError("Import failed. File alCall ready exists: " & dataLoc & "\Commands\" & lines(2) & ".ebacmd","COMMAND_ALCall readY_EXISTS")
					Else
				...
				End If
			Else
				Call giveError("Something is wrong with that file. EBA Command Center couldn't Call read it or couldn't understand its contents: " & importData,"INVALID_IMPORT_FILE")
			End If
			...
			If eba = vbYes Then
				...
				If eba = "user" or eba = "cmd" or eba = "settings" or eba = "plug" Then
					...
					Elseif eba = "plug" Then
						...
						backup1.CopyHere(backup2.Items)
						If Err.Number = 0 Then
							Call giveNote("Restored files to " & dataLoc & "\Plugins")
						Else
							Call giveError("Restore failed. See WScript Error for more info.","WS/" & Err.Number)
						End If
						Call checkWScript
...
End Sub
Sub giveCritical(msg,code)
	On Error Resume Next
	Call addError
	msgbox "Critical:" & line & msg & line & "Error code: " & code,16,title
End Sub
...
End Sub
Sub giveInternal(msg,code)
	On Error Resume Next
	Call addError
	msgbox "Internal Exception:" & line & msg & line & "Error code: " & code,48,title
End Sub
Sub giveNote(msg)
	On Error Resume Next
	Call addNote
...
Sub readCommands(baseDir)
	...
	Elseif LCase(lines(2)) = "script" Then
		...
		Else
			Call giveError("The command references a script that does not exist.","FILE_NOT_FOUND")
		End If
	Else
		Call giveError("That command contains invalid data or is corrupt.","INVALID_COMMAND")
	End If
	...
	For forVar = 0 to 9
		...
		If foldExists(temp(0) & "\Scripts.vbs") Then
			...
			For Each forVar1 In fs.GetFolder(temp(0) & "\Scripts.vbs").Files
				...
				Elseif LCase(forVar1.name) <> "desktop.ini" Then
					Call giveAlert("Internal Exception in Plugin " & temp(0) & line & "Location: Scripts.vbs\" & forVar1.name & vblf & "Error Generated: ScriptVBSEncounteredNonVBS" & vblf & "What this means: The script could not be loaded by EBA Command Center because Script.vbs only supports VBS files." & line & "This script will not execute when referenced.")
				End If
			Next
		End If
		If foldExists(temp(0) & "\Scripts.js") Then
			...
			If foldExists(temp(0) & "\Scripts.js\Shutdown") Then
				For Each forVar1 In fs.GetFolder(temp(0) & "\Scripts.js\Shutdown").Files
					If LCase(Right(forVar1, 3)) = ".js" Then
						...
						fs.CopyFile forVar1, dataLoc & "\PluginData\Scripts\Shutdown\" & forVar1.name
					Elseif LCase(forVar1.name) <> "desktop.ini" Then
						Call giveAlert("Internal Exception in Plugin " & temp(0) & line & "Location: Scripts.js\Shutdown\" & forVar1.name & vblf & "Error Generated: ScriptJSEncounteredNonJS" & vblf & "What this means: The script could not be loaded by EBA Command Center because Script.js only supports JS files." & line & "This script will not execute on shutdown.")
					End If
				Next
			End If
			For Each forVar1 In fs.GetFolder(temp(0) & "\Scripts.js").Files
				If LCase(Right(forVar1, 3)) = ".js" Then
					...
					fs.CopyFile forVar1, dataLoc & "\PluginData\Scripts\" & forVar1.name
				Elseif LCase(forVar1.name) <> "desktop.ini" Then
					Call giveAlert("Internal Exception in Plugin " & temp(0) & line & "Location: Scripts.js\" & forVar1.name & vblf & "Error Generated: ScriptJSEncounteredNonJS" & vblf & "What this means: The script could not be loaded by EBA Command Center because Script.js only supports JS files." & line & "This script will not execute when referenced.")
				End If
			Next
		End If
		If foldExists(temp(0) & "\Scripts.exe") Then
			...
			For Each forVar1 In fs.GetFolder(temp(0) & "\Scripts.exe").Files
				If LCase(Right(forVar1, 4)) = ".exe" Then
					...
					fs.CopyFile forVar1, dataLoc & "\PluginData\Scripts\" & forVar1.name
				Elseif LCase(forVar1.name) <> "desktop.ini" Then
					Call giveAlert("Internal Exception in Plugin " & temp(0) & line & "Location: Scripts.exe\" & forVar1.name & vblf & "Error Generated: ScriptEXEEncounteredNonEXE" & vblf & "What this means: The script could not be loaded by EBA Command Center because Script.exe only supports EXE files." & line & "This script will not execute when referenced.")
				End If
			Next
		End If
