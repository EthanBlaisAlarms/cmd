...
Dim oldDataLoc : oldDataLoc = "C:\ProgramData\EBA"
...
enableEndOpMenu = cmd.RegRead("HKLM\SOFTWARE\EBA-Cmd\enableOperationCompletedMenu")
enableEndOpMenu = CInt(enableEndOpMenu)
...
If LCase(Right(importData, 10)) = ".ebaimport" Or LCase(Right(importData, 10)) = ".ebabackup" Then
	eba = msgbox("EBA Command Center detected an import request. Review this request?",4+48+4096,title)
	If eba = vbYes And LCase(Right(importData, 10)) = ".ebaimport" Then
		Call readLines(importData,1)
		If lines(1) = "Type: Startup Key" Then
			Call readLines(importData,2)
			If lines(2) = "Data: eba.recovery" Then
				eba = msgbox("Start EBA Command Center in recovery mode?",4+32+4096,title)
		...
		Elseif lines(1) = "Type: Command" Then
			...
			eba = msgbox("Import this command?" & line & "Name: " & lines(2) & vblf & "Type: " & lines(3) & vblf & "Target: " & lines(4) & vblf & "Require Login: " & lines(5),4+32+4096,title)
			If eba = vbYes Then
	...
	Elseif eba = vbYes And LCase(Right(importData, 10)) = ".ebabackup" Then
		eba = msgbox("This seems to be a backup file. Do you want to add this file to your installation? Any duplicate files will be overwritten by the backup.", 4+32+4096, title)
		If eba = vbYes Then
			eba = LCase(inputbox("What type of backup is this? This ensures data is extracted correctly." & line & "'USER': Backs up user accounts." & vblf & "'CMD': Backs up commands." & vblf & "'SETTINGS': Backs up settings.",title))
			If eba = "user" or eba = "cmd" or eba = "settings" Then
				...
				If eba = "user" Then
					...
					Else
						Call giveError("Restore failed. See WScript Error for more info.")
					End If
					Call checkWScript
				Elseif eba = "cmd" Then
...
Do
	If startupType = "normal" Then
		...
		Call dataExistsW(dataLoc & "\Commands\backup.ebacmd")
			...
			Else
				...
				Else
					...
					Else
						curEdit = "basic"
						title = "EBA Command Center Basic " & ver
						eba = msgbox("Warning:" & line & "Failed to download EBA Keys from our servers. You can continue, but you'll use EBA Basic until we can access our servers. Would you like to retry?",4+48+4096,title)
		...
		Else
			Call addWarn
			eba = msgbox("Warning:" & line & "We ran into a problem checking for updates. Retry?",4+48+4096,title)
			If eba = vbNo Then
		...
		If Not missFiles = false Then
			skipDo = True
			eba = msgbox("EBA Command Center did not start correctly." & line & "ABORT: Close EBA Command Center" & vblf & "RETRY: Try again" & vblf & "IGNORE: Ignore issue and continue to recovery.",2+48+4096,title)
			If eba = vbIgnore Then
				...
				Elseif eba = "start" Then
					eba = msgbox("Warning:" & line & "EBA Command Center didnt start correctly. We recommend running recovery options instead of starting. Continue anyways?",4+48+4096,title)
		...
		If skipDo = False Then
			...
			If secureShutdown = "false" Then
				msgbox "EBA Command Center failed to shut down correctly last time it was used. Make sure you shut down EBA Command Center correctly next time!", 48+4096, "EBA Crash Handler"
				Call endOp("n")
			End If
			
			'EBA Command Center Runtime
			eba = msgbox("Start " & title & "?", 4+32+4096, title)
		...
		Do
			...
			If Err.Number > 0 Then
				Call giveError("A critical error occurred in EBA Command Center. EBA Command Center will now crash.")
				Call endOp("c")
			End If
			Call dataExists(programLoc & "\EBA.vbs")
			Call dataExists(dataLoc & "\isLoggedIn.ebacmd")
			...
			If Not missFiles = false Then
				eba = msgbox("EBA Command Center ran into a problem it couldn't handle. We recommend closing EBA Command Center to prevent damage to EBA data. Close now?",4+16+4096,"EBA Crash Handler")
				If eba = vbYes Then
					Call endOp("c")
				End If
			End If
			...
			Elseif exeValue = "eba.backup" Then
				...
				If curEdit = "ent" Then
					...
					If eba = vbYes Then
						...
						If eba = "user" or eba = "cmd" or eba = "settings" Then
							...
							If eba = "user" Then
								...
								Else
									Call giveError("Backup failed. See WScript Error for more info.")
								End If
								Call checkWScript
							Elseif eba = "cmd" Then
					...
					End If
				Else
					Call giveError("This feature requires EBA Enterprise! Upgrade with the 'upgrade' command.")
				End If
	...
	Elseif startupType = "repair" Then
		...
		programLoc = cmd.RegRead("HKLM\SOFTWARE\EBA-Cmd\installDir")
		Err.Clear
		eba = msgbox("Are you sure you want to run automatic repair? Running automatic repair will reset your preferences.",4+48+4096,title)
		If eba = vbYes Then
	...
	Elseif startupType = "install" Then
		...
		If isInstalled = True Then
			eba = msgbox("A legacy EBA Command Center (6.1 and below) installation was found on your system. When you update, this installation will be erased. Continue?", 4+48+4096, title)
			If eba = vbNo Then Call endOp("c")
		End If
		...
		'Edition
		eba = msgbox("Would you like to install the free version?",4+32+4096,title)
		If eba = vbNo Then
			...
			Elseif eba = "help" Then
				...
				msgbox "EBA Command Center Pro contains:" & line & "Everything from EBA Basic" & vblf & "3 User Accounts" & vblf & "No Ads" & vblf & "Creation of Cmd and Short commands." & vblf & line & "Recommended for Shared Computers/Developers",64+4096,title
				msgbox "EBA Command Center Enterprise contains:" & line & "Everything from EBA Basic and EBA Pro" & vblf & "100 User Accounts" & vblf & "Backups" & vblf & line & "Recommended for Businesses",64+4096,title
		...
		eba = msgbox("Create a Desktop icon?", 4+32+4096, title)
		If eba = vbYes Then
			...
			End With
		End If
		eba = msgbox("Create a Start Menu icon?", 4+32+4096, title)
		If eba = vbYes Then
			newFolder(startMenu)
			...
			End With
		End If
		eba = msgbox("EBA Command Center has finished installing! Would you like to start setup?",4+64+4096,title)
		If eba = vbYes Then Call endOp("r")
		Call endOp("c")
	Elseif startupType = "update" Then
		...
		eba = msgbox("EBA Command Center is already installed on this device. Update now?",4+32+4096, title)
		If eba = vbNo Then Call endOp("f")
		...
		'Folders
		newFolder(programLoc)
		If foldExists(programLoc & "\Plugins") Then fs.DeleteFolder(programLoc & "\Plugins")
		newFolder(dataLoc)
		newFolder(dataLoc & "\Users")
		newFolder(dataLoc & "\Commands")
		newFolder(dataLoc & "\Settings")
		If foldExists(oldDataLoc) Then
			eba = msgbox("Warning:" & line & "EBA Command Center detected data in the old data location (" & oldDataLoc & "), which is your AppData folder. Data will be moved to " & dataLoc & ", but the data currently in " & dataLoc & " will be removed. Would you like to move your old data into the new folder?",4+48+4096,title)
			Call addError
			If eba = vbYes Then
	...
	Elseif startupType = "refresh" Then
		...
		eba = msgbox("Reinstalling EBA Command Center can fix issues you might have. On the next screen, you will choose what to keep. Continue with reinstallation?", 4+48+4096, title)
		If eba = vbNo Then
			Call write(dataLoc & "\startupType.ebacmd","normal")
			Call endOp("r")
		End If
		
		eba = msgbox("By refreshing EBA Command Center on this device, you understand that we cannot be held responsible for what you choose to do with EBA Command Center.",4+64+4096, title)
		If eba = vbNo Then
		...
		If eba = "keep" or eba = "refresh" or eba = "full" Then
			...
			Elseif eba = "refresh" Then
				temp(1) = "yes"
			End If
			eba = msgbox("Preserved data:" & line & "Script: yes" & vblf & "Users: " & temp(0) & vblf & "Commands: " & temp(1) & vblf & "Settings: " & temp(0) & line & "Proceed? This cannot be undone", 4+48+4096, title)
			If eba = vbNo Then
				Call write(dataLoc & "\startupType.ebacmd","normal")
				Call endOp("c")
			End If
		...
		eba = msgbox("Would you like to reinstall EBA Command Center to " & programLoc & "?",4+32+4096,title)
		If eba = vbNo Then
			Do
	...
	Elseif startupType = "firstrun" Then
		...
		eba = msgbox("Do you want to enable EBA Logs (Logs your EBA Command Center data)?" & vblf & "**Recommended**",4+32+4096,title)
		If eba = vbYes Then
			Call write(dataLoc & "\settings\logging.ebacmd","true")
		Else
			Call write(dataLoc & "\settings\logging.ebacmd","false")
		End If
		eba = msgbox("Do you want to save your login information when EBA Command Center closes?" & vblf & "**Not Recommended**",4+32+4096,title)
		If eba = vbYes Then
			Call write(dataLoc & "\settings\saveLogin.ebacmd","true")
		Else
			Call write(dataLoc & "\settings\saveLogin.ebacmd","false")
	...
	Elseif startupType = "firstrepair" Then
		...
		eba = msgbox("Run initial setup?",4+32+4096,title)
		If eba = vbYes Then
...
Sub endOp(args)
	...
	End If
	count(0) = count(0) + 1
	If enableEndOpMenu = 1 Then msgbox "Operation " & count(0) & " Completed:" & nls & "Errors: " & count(3) & vblf & "Warnings: " & count(2) & vblf & "Notices: " & count(1), 64+4096, title
	Call clearCounts
	Call clearTemps
	Call clearLines
...
Sub clearLines
	...
	End Sub
Sub giveError(msg)
	msgbox "Error:" & line & msg, 16+4096, title
	Call addError
	End Sub
Sub giveWarn(msg)
	msgbox "Warning:" & line & msg, 48+4096, title
	Call addWarn
	End Sub
Sub giveNote(msg)
	msgbox "Notice:" & line & msg, 64+4096, title
	Call addNote
	End Sub
...
Sub createUninstallFile
	...
	temp(0) = temp(0) & vblf & "Dim objApp : Set objApp = CreateObject(""Shell.Application"")"
	...
	temp(0) = temp(0) & vblf & "eba = msgbox(""EBA Command Center needs to be ran as administrator to continue uninstallation."",32+4096,title)"
	temp(0) = temp(0) & vblf & "objApp.ShellExecute ""wscript.exe"", Chr(34) & Wscript.scriptfullname & Chr(34), """", ""runas"", 1"
	temp(0) = temp(0) & vblf & "wscript.quit"
	temp(0) = temp(0) & vblf & "End If"
	temp(0) = temp(0) & vblf & "title = ""EBA Command Center Uninstallation"""
	temp(0) = temp(0) & vblf & "eba = msgbox(""EBA Command Center is about to be uninstalled. Continue?"",4+48+4096,""Warning"")"
	temp(0) = temp(0) & vblf & "If eba = vbNo Then"
	temp(0) = temp(0) & vblf & "msgbox ""Your EBA Command Center data has been restored."",64+4096,""Important"""
	...
	temp(0) = temp(0) & vblf & "msgbox ""EBA Command Center was uninstalled."",48+4096,title"
	temp(0) = temp(0) & vblf & "End if"
...
Sub checkWscript
	If Not Err.Number = 0 Then
		...
		If Err.Number = -2147024894 Then
			temp(0) = "Something went wrong accessing a file/registry key on your system."
		Elseif Err.Number = -2147024891 Then
			temp(0) = "Failed to access your system registry."
		Elseif Err.Number = -2147483638 Then
			temp(0) = "Failed to download data from the EBA Website."
...
Sub getKeys(editionToGetKeysFrom)
	...
	If Err.Number > 0 Then
		...
		If Not startupType = "normal" Then
			eba = msgbox("Warning:" & line & "We ran into a problem downloading the EBA Keys. This is required to continue. Retry?",4+48+4096,title)
			If eba = vbNo Then
				Call endOp("c")
			Else
				Err.Clear
...
'
'
'File Formats (%EBA% is C:\ProgramData\EBA)
'
'.ebacmd User File
