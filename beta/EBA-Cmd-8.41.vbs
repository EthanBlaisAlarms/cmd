'EBA Command Center 8.41   |   Owned by EBA Tools
...
Dim endOpFail : endOpFail = False

'Arrays
Dim temp(9), count(4), lines(5), loadedPlugins(9), localization(99)
Call clearTemps
Call clearCounts
Call clearLines
Call resetLocalization
count(0) = 0
count(4) = 0
...
If fExists(cmd.SpecialFolders("Startup") & "\uninstallEBA.vbs") Then
	Error localization(0),"UNINSTALLION_SCHEDULED"
	Call endOp("c")
End If
...
If InStr(temp(0),"microsoft") Then
	If InStr(temp(0),"windows") Then
		...
		Elseif InStr(temp(0),"vista") Then
			Warn localization(1)
		Else
			Error localization(2),"Invalid_Windows_Version"
		End If
	Else
		Error localization(3),"Windows_RE"
		Call endOp("c")
	End If
Else
	Error localization(4),"INVALID_OS"
	Call endOp("c")
End If
...
If scriptRunning() Then
	Error localization(0),"EBA_ALREADY_RUNNING"
	Call endOp("c")
End If
If checkCScript() Then
	Error localization(0),"USE_WSCRIPT_NOT_CSCRIPT"
	Call endOp("s")
End If

delete(dataLoc & "\PluginData\Scripts")
delete(dataLoc & "\PluginData\Commands")
delete(dataLoc & "\PluginData\Files")
...
Do
	...
	Elseif startupType = "update" Then
		Call modeUpdate
	Else
		eba = msgbox(localization(5) & line & locatization(6) & startupType & localization(7),4+48,title)
		If eba = vbYes Then
...
Sub modeFirstrepair
	On Error Resume Next
	title = localization(8) & ver & localization(9) & localization(10)
	Call checkWScript
	...
	Do
		...
		Elseif fExists(dataLoc & "\Users\" & eba & ".ebacmd") Then
			...
			Else
				warn "That User Account exists, but the account is either corrupt, or was not the original account."
			End If
		Else
...
Sub modeInstall
	...
	If foldExists("C:\EBA") Or foldExists("C:\EBA-Installer") Then
		Warn("A legacy EBA Cmd Installation (6.1 and below) was found on this device. Note that when you install EBA Cmd " & ver & ", this version will be uninstalled.")
	End If
	...
	'Install Directory
	programLoc = inputbox("We're Call ready to install EBA Command Center! Where would you like to install to? Make sure this folder is empty.",title,programLoc)
...
Sub modeNormal
	...
	If skipDo = False Then		
		...
		If secureShutdown = "false" Then
			Alert("EBA Command Center did not shut down without executing shutdown actions. Make sure to shut down EBA Command Center correctly next time.")
			Call endOp("n")
		End If
	...
	Do
			...
			Elseif eba = "acc" or eba = "account" Then
				...
				If eba = "new" Then
					...
					If temp(0) < temp(1) Then
						...
						End If
					Else
						Error "Your current edition of EBA Command Center has an account limit of " & temp(1) & ". You are using " & temp(0) & " accounts, and cannot add more.","INCORRECT_EDITION"
					End If
				Elseif eba = "edit" Then
		...
		Elseif exeValue = "eba.login" Then
			...
			If fExists(dataLoc & "\Users\" & uName & ".ebacmd") Then
				...
				End If
			Else
				Error "Username not found.","USERNAME_NOT_FOUNT"
			End If
		...
		Elseif exeValue = "eba.reset" Then
			...
			If eba = vbYes Then
				...
				If eba = vbYes Then
					cmd.run "systemreset"
					Warn("Your PC is being reset. Follow all on-screen prompts. Press OK to cancel.")
				End If
			End If
...
Sub checkImports
	...
	If LCase(Right(importData, 10)) = ".ebaimport" Or LCase(Right(importData, 10)) = ".ebabackup" Or LCase(Right(importData, 10)) = ".ebaplugin" Then
		If LCase(Right(importData, 10)) = ".ebaimport" Then
			...
			If lines(1) = "Type: Startup Key" Then
				...
				Else
					Error "Something is wrong with that file. EBA Command Center couldn't Call read it or couldn't understand its contents: " & importData,"INVALID_STARTUP_KEY"
				End If
			Elseif lines(1) = "Type: Command" Then
				...
				If eba = vbYes Then
					If fExists(dataLoc & "\Commands\" & lines(2) & ".ebacmd") Then
						Error "Import failed. File already exists: " & dataLoc & "\Commands\" & lines(2) & ".ebacmd","COMMAND_ALREADY_EXISTS"
					Else
				...
				End If
			Else
				Error "Something is wrong with that file. EBA Command Center couldn't Call read it or couldn't understand its contents: " & importData,"INVALID_IMPORT_FILE"
			End If
	...
	Elseif importData = "" Then
		importData = False
	Else
		Error "Something is wrong with your imported file. EBA Command Center cannot import this file." & line & importData,"EXT_NOT_EBAIMPORT_OR_EBABACKUP"
	End If
End Sub
Sub checkWScript
	temp(8) = Err.Number
	...
	For Each forVar In fs.GetFolder(dataLoc & "\Plugins").Subfolders
		If fExists(forVar & "\meta.xml") Then
			...
			For Each forVar1 In objXML.selectNodes("/Meta/Format")
				Call checkWScript
				If forVar1.text = "1" Then
					For Each forVar2 In objXML.selectNodes("/Meta/License/ID")
						Call checkWScript
...
Sub read(dir,arg)
	...
	End If
End Sub
Sub resetLocalization
	localization(0) = "Cannot start EBA Command Center."
	localization(1) = "Windows Vista might not support EBA Command Center."
	localization(2) = "Your version of Windows does not support EBA Command Center."
	localization(3) = "EBA Command Center will not run in Windows Recovery. Please boot into Windows to use EBA Cmd."
	localization(4) = "EBA Command Center will not run on your OS. EBA Command Center is designed for Windows 10, but will support as early as Windows 7."
	localization(5) = "Warning:"
	localization(6) = "There is a problem with startupType "
	localization(7) = ". Do you want to reset it?"
	localization(8) = "EBA Cmd "
	localization(9) = " | "
	localization(10) = "EBA Repair"
End Sub
Sub runPlugins
	Call clearTemps
	For forVar = 0 to 9
		...
		If foldExists(temp(0) & "\Commands") Then
			...
			For Each forVar1 In fs.GetFolder(temp(0) & "\Commands").Files
				objXML.load(temp(0) & "\Commands\" & forVar1.name)
				For Each forVar2 In objXML.selectNodes("/Command/Format")
					If forVar2.text = "1" Then
						For Each forVar3 In objXML.selectNodes("/Command/Target")
							temp(1) = forVar3.Text
						Next
						For Each forVar3 In objXML.selectNodes("/Command/Type")
							temp(2) = forVar3.text
						Next
						For Each forVar3 In objXML.selectNodes("/Command/Login")
							temp(3) = forVar3.text
						Next
						Call write(dataLoc & "\PluginData\Commands\" & Replace(forVar1.name,".xml","") & ".ebacmd",temp(1) & vblf & temp(2) & vblf & temp(3) & vblf & "no")
					Else
						Alert("Internal Exception in Plugin " & temp(0) & line & "Location: Commands\" & forVar1.name & vblf & "Error Generated: <Command>/<Format>***ERR_INVAL***</Format>\</Command>" & vblf & "What this means: The value at /Command/Format is invalid." & line & "This XML will be skipped.")
					End If
				Next
			Next
		End If
		If foldExists(temp(0) & "\Scripts.vbs") Then
			...
			If foldExists(temp(0) & "\Scripts.vbs\Shutdown") Then
				For Each forVar1 In fs.GetFolder(temp(0) & "\Scripts.vbs\Shutdown").Files
					...
					Elseif LCase(forVar1.name) <> "desktop.ini" Then
						Alert("Internal Exception in Plugin " & temp(0) & line & "Location: Scripts.vbs\Shutdown\" & forVar1.name & vblf & "Error Generated: ScriptVBSEncounteredNonVBS" & vblf & "What this means: The script could not be loaded by EBA Command Center because Script.vbs only supports VBS files." & line & "This script will not execute on shutdown.")
					End If
				Next
			End If
			For Each forVar1 In fs.GetFolder(temp(0) & "\Scripts.vbs").Files
				...
				Elseif LCase(forVar1.name) <> "desktop.ini" Then
					Alert("Internal Exception in Plugin " & temp(0) & line & "Location: Scripts.vbs\" & forVar1.name & vblf & "Error Generated: ScriptVBSEncounteredNonVBS" & vblf & "What this means: The script could not be loaded by EBA Command Center because Script.vbs only supports VBS files." & line & "This script will not execute when referenced.")
				End If
			Next
		End If
		If foldExists(temp(0) & "\Scripts.js") Then
			...
			If foldExists(temp(0) & "\Scripts.js\Shutdown") Then
				For Each forVar1 In fs.GetFolder(temp(0) & "\Scripts.js\Shutdown").Files
					If LCase(Right(forVar1, 3)) = ".js" Then
						...
						fs.CopyFile forVar1, dataLoc & "\PluginData\Scripts\Shutdown\" & forVar1.name
					Elseif LCase(forVar1.name) <> "desktop.ini" Then
						Alert("Internal Exception in Plugin " & temp(0) & line & "Location: Scripts.js\Shutdown\" & forVar1.name & vblf & "Error Generated: ScriptJSEncounteredNonJS" & vblf & "What this means: The script could not be loaded by EBA Command Center because Script.js only supports JS files." & line & "This script will not execute on shutdown.")
					End If
				Next
			End If
			For Each forVar1 In fs.GetFolder(temp(0) & "\Scripts.js").Files
				If LCase(Right(forVar1, 3)) = ".js" Then
					...
					fs.CopyFile forVar1, dataLoc & "\PluginData\Scripts\" & forVar1.name
				Elseif LCase(forVar1.name) <> "desktop.ini" Then
					Alert("Internal Exception in Plugin " & temp(0) & line & "Location: Scripts.js\" & forVar1.name & vblf & "Error Generated: ScriptJSEncounteredNonJS" & vblf & "What this means: The script could not be loaded by EBA Command Center because Script.js only supports JS files." & line & "This script will not execute when referenced.")
				End If
			Next
		End If
		If foldExists(temp(0) & "\Scripts.exe") Then
			...
			If foldExists(temp(0) & "\Scripts.exe\Shutdown") Then
				For Each forVar1 In fs.GetFolder(temp(0) & "\Scripts.exe\Shutdown").Files
					If LCase(Right(forVar1, 4)) = ".exe" Then
						...
						fs.CopyFile forVar1, dataLoc & "\PluginData\Scripts\Shutdown\" & forVar1.name
					Elseif LCase(forVar1.name) <> "desktop.ini" Then
						Alert("Internal Exception in Plugin " & temp(0) & line & "Location: Scripts.exe\Shutdown\" & forVar1.name & vblf & "Error Generated: ScriptEXEEncounteredNonEXE" & vblf & "What this means: The script could not be loaded by EBA Command Center because Script.exe only supports EXE files." & line & "This script will not execute on shutdown.")
					End If
				Next
			End If
			For Each forVar1 In fs.GetFolder(temp(0) & "\Scripts.exe").Files
				If LCase(Right(forVar1, 4)) = ".exe" Then
					...
					fs.CopyFile forVar1, dataLoc & "\PluginData\Scripts\" & forVar1.name
				Elseif LCase(forVar1.name) <> "desktop.ini" Then
					Alert("Internal Exception in Plugin " & temp(0) & line & "Location: Scripts.exe\" & forVar1.name & vblf & "Error Generated: ScriptEXEEncounteredNonEXE" & vblf & "What this means: The script could not be loaded by EBA Command Center because Script.exe only supports EXE files." & line & "This script will not execute when referenced.")
				End If
			Next
		End If
		If foldExists(temp(0) & "\Files") Then
			...
			Next
		End If
		If fExists(temp(0) & "\Localization.xml") Then
			objXML.load(temp(0) & "\Localization.xml")
			For Each forVar1 In objXML.selectNodes("/Error/CannotStart")
				If forVar1.text <> "" Then localization(0) = forVar1.text
			Next
			For Each forVar1 In objXML.selectNodes("/Error/WindowsVista")
				If forVar1.text <> "" Then localization(1) = forVar1.text
			Next
			For Each forVar1 In objXML.selectNodes("/Error/InvalidWindows")
				If forVar1.text <> "" Then localization(2) = forVar1.text
			Next
			For Each forVar1 In objXML.selectNodes("/Error/WindowsRE")
				If forVar1.text <> "" Then localization(3) = forVar1.text
			Next
			For Each forVar1 In objXML.selectNodes("/Error/InvalidOS")
				If forVar1.text <> "" Then localization(4) = forVar1.text
			Next
			For Each forVar1 In objXML.selectNodes("/Header/Warning")
				If forVar1.text <> "" Then localization(5) = forVar1.text
			Next
			For Each forVar1 In objXML.selectNodes("/Error/InvalidStartupType")
				If forVar1.text <> "" Then localization(6) = forVar1.text
			Next
			For Each forVar1 In objXML.selectNodes("/Error/InvalidStartupType2")
				If forVar1.text <> "" Then localization(7) = forVar1.text
			Next
			For Each forVar1 In objXML.selectNodes("/Header/AppName")
				If forVar1.text <> "" Then localization(8) = forVar1.text
			Next
			For Each forVar1 In objXML.selectNodes("/Header/Div")
				If forVar1.text <> "" Then localization(9) = forVar1.text
			Next
			For Each forVar1 In objXML.selectNodes("/Header/Mode/FirstRepair")
				If forVar1.text <> "" Then localization(10) = forVar1.text
			Next
		End If
	Next
End Sub
...
Sub update(dir,writeData,arg)
	On Error Resume Next
	If LCase(arg) = "overCall write" Then
		Call write(dir,writeData)
	Elseif LCase(arg) = "Call append" Then
...
Sub updateCommands
	...
	Call update(fileDir,"eba.admin","overCall write")
	Call update(fileDir,"exe","Call append")
	Call update(fileDir,"no","Call append")
	Call update(fileDir,"builtin","Call append")
	delete(dataLoc & "\Commands\admin.ebacmd")
	
	fileDir = programLoc & "\Commands\backup.ebacmd"
	Call update(fileDir,"eba.backup","overCall write")
	Call update(fileDir,"exe","Call append")
	Call update(fileDir,"no","Call append")
	Call update(fileDir,"builtin","Call append")
	...
	Call update(fileDir,"eba.config","overCall write")
	Call update(fileDir,"exe","Call append")
	Call update(fileDir,"yes","Call append")
	Call update(fileDir,"builtin","Call append")
	delete(dataLoc & "\Commands\config.ebacmd")
	
	fileDir = programLoc & "\Commands\crash.ebacmd"
	Call update(fileDir,"eba.crash","overCall write")
	Call update(fileDir,"exe","Call append")
	Call update(fileDir,"no","Call append")
	Call update(fileDir,"builtin","Call append")
	...
	Call update(fileDir,"eba.dev","overCall write")
	Call update(fileDir,"exe","Call append")
	Call update(fileDir,"no","Call append")
	Call update(fileDir,"builtin","Call append")
	...
	Call update(fileDir,"eba.end","overCall write")
	Call update(fileDir,"exe","Call append")
	Call update(fileDir,"no","Call append")
	Call update(fileDir,"builtin","Call append")
	...
	Call update(fileDir,"eba.error","overCall write")
	Call update(fileDir,"exe","Call append")
	Call update(fileDir,"no","Call append")
	Call update(fileDir,"builtin","Call append")
	...
	Call update(fileDir,"eba.export","overCall write")
	Call update(fileDir,"exe","Call append")
	Call update(fileDir,"no","Call append")
	Call update(fileDir,"builtin","Call append")
	...
	Call update(fileDir,"eba.help","overCall write")
	Call update(fileDir,"exe","Call append")
	Call update(fileDir,"no","Call append")
	Call update(fileDir,"builtin","Call append")
	...
	Call update(fileDir,"eba.import","overCall write")
	Call update(fileDir,"exe","Call append")
	Call update(fileDir,"no","Call append")
	Call update(fileDir,"builtin","Call append")
	...
	Call update(fileDir,"eba.login","overCall write")
	Call update(fileDir,"exe","Call append")
	Call update(fileDir,"no","Call append")
	Call update(fileDir,"builtin","Call append")
	...
	Call update(fileDir,"eba.logout","overCall write")
	Call update(fileDir,"exe","Call append")
	Call update(fileDir,"no","Call append")
	Call update(fileDir,"builtin","Call append")
	...
	Call update(fileDir,logDir,"overCall write")
	Call update(fileDir,"file","Call append")
	Call update(fileDir,"no","Call append")
	Call update(fileDir,"builtin","Call append")
	delete(dataLoc & "\Commands\logs.ebacmd")
	
	fileDir = programLoc & "\Commands\plugin.ebacmd"
	Call update(fileDir,"eba.plugin","overCall write")
	Call update(fileDir,"exe","Call append")
	Call update(fileDir,"no","Call append")
	Call update(fileDir,"builtin","Call append")
	
	fileDir = programLoc & "\Commands\Call read.ebacmd"
	Call update(fileDir,"eba.Call read","overCall write")
	Call update(fileDir,"exe","Call append")
	Call update(fileDir,"no","Call append")
	Call update(fileDir,"builtin","Call append")
	delete(dataLoc & "\Commands\Call read.ebacmd")
	
	fileDir = programLoc & "\Commands\refresh.ebacmd"
	Call update(fileDir,"eba.refresh","overCall write")
	Call update(fileDir,"exe","Call append")
	Call update(fileDir,"yes","Call append")
	Call update(fileDir,"builtin","Call append")
	...
	Call update(fileDir,"eba.restart","overCall write")
	Call update(fileDir,"exe","Call append")
	Call update(fileDir,"no","Call append")
	Call update(fileDir,"builtin","Call append")
	...
	Call update(fileDir,"sys.run","overCall write")
	Call update(fileDir,"exe","Call append")
	Call update(fileDir,"no","Call append")
	Call update(fileDir,"builtin","Call append")
	...
	Call update(fileDir,"sys.shutdown","overCall write")
	Call update(fileDir,"exe","Call append")
	Call update(fileDir,"no","Call append")
	Call update(fileDir,"builtin","Call append")
	...
	Call update(fileDir,"eba.uninstall","overCall write")
	Call update(fileDir,"exe","Call append")
	Call update(fileDir,"yes","Call append")
	Call update(fileDir,"builtin","Call append")
	...
	Call update(fileDir,"https://ethanblaisalarms.github.io/cmd","overCall write")
	Call update(fileDir,"url","Call append")
	Call update(fileDir,"no","Call append")
	Call update(fileDir,"builtin","Call append")
	delete(dataLoc & "\Commands\update.ebacmd")
	
	fileDir = programLoc & "\Commands\upgrade.ebacmd"
	Call update(fileDir,"eba.upgrade","overCall write")
	Call update(fileDir,"exe","Call append")
	Call update(fileDir,"yes","Call append")
	Call update(fileDir,"builtin","Call append")
	...
	Call update(fileDir,"eba.version","overCall write")
	Call update(fileDir,"exe","Call append")
	Call update(fileDir,"no","Call append")
	Call update(fileDir,"builtin","Call append")
	...
	fileDir = programLoc & "\Commands\Call write.ebacmd"
	Call update(fileDir,"eba.Call write","overCall write")
	Call update(fileDir,"exe","Call append")
	Call update(fileDir,"no","Call append")
	Call update(fileDir,"builtin","Call append")
	delete(dataLoc & "\Commands\Call write.ebacmd")
End Sub
...
'' SIG '' MIIDQAYJKoZIhvcNAQcCoIIDMTCCAy0CAQExCzAJBgUr
'' SIG '' DgMCGgUAMGcGCisGAQQBgjcCAQSgWTBXMDIGCisGAQQB
'' SIG '' gjcCAR4wJAIBAQQQTvApFpkntU2P5azhDxfrqwIBAAIB
'' SIG '' AAIBAAIBAAIBADAhMAkGBSsOAwIaBQAEFH7ixsgkYprV
'' SIG '' 2F5eWXfGOPxM3oBdoIIBkzCCAY8wggEVoAMCAQICCCG1
'' SIG '' QVxWqvCNMAoGCCqGSM49BAMDMC8xLTArBgNVBAMTJGIx
'' SIG '' NDc1NzRmLWYxZDUtNDkzMS1iN2VhLTllMzM1Mzk0MTg1
'' SIG '' YTAeFw0yMTAzMjcwODI5MTZaFw0yMjAzMjcyMDI5MTZa
'' SIG '' MC8xLTArBgNVBAMTJGIxNDc1NzRmLWYxZDUtNDkzMS1i
'' SIG '' N2VhLTllMzM1Mzk0MTg1YTB2MBAGByqGSM49AgEGBSuB
'' SIG '' BAAiA2IABAJbOH+/mhlqwv8m6tvNKtytUnPdjPkQocZy
'' SIG '' zAHTUZyEB8JX4o/ZjBR0w2UrpD3BGC2FrydHkW9x8lhk
'' SIG '' Indh+EMyWhn8LOD4wuigxGb+fQUSZINfZFVQC1DTCwzR
'' SIG '' H8BcUzAKBggqhkjOPQQDAwNoADBlAjEAnXw+JiGhkbvI
'' SIG '' tpKdBWksTBJJeo5ZusVYUlFsLlffknSFTHvPJ9RVHRuA
'' SIG '' A8EyhiFFAjBhemv9Z1EwTmpw1Io7mX7t4OBZeYE1pjLH
'' SIG '' p39KXWzzIGcPUJ/9K+zHG/mdYgLAcoIxggEZMIIBFQIB
'' SIG '' ATA7MC8xLTArBgNVBAMTJGIxNDc1NzRmLWYxZDUtNDkz
'' SIG '' MS1iN2VhLTllMzM1Mzk0MTg1YQIIIbVBXFaq8I0wCQYF
'' SIG '' Kw4DAhoFAKBSMBAGCisGAQQBgjcCAQwxAjAAMBkGCSqG
'' SIG '' SIb3DQEJAzEMBgorBgEEAYI3AgEEMCMGCSqGSIb3DQEJ
'' SIG '' BDEWBBQAlvY6bGcG5OkDuCNyzgQqwAiksTALBgcqhkjO
'' SIG '' PQIBBQAEZzBlAjBDLmeqmokDGmqdC05D/qlfEDz6+WRc
'' SIG '' 8LMNeaDCOMrca7nCbgGMDNAS3mrqYszASjsCMQCGRLxf
'' SIG '' EWZXabNUb4hVPhqz0d6whSkzyYxHZeB9y9sAULvDvPQj
'' SIG '' 2Z8BMkA43dr62Pg=
